...
class BoatDesign {
  ...
  static getHeightRatioFromDesignY( y ) {
    return Utils.linear( -BoatDesign.BOAT_HEIGHT, 0, 0, 1, y );
  }
  ...
  static getControlPoints( heightRatio, isInside ) {
    ...
    if ( !isInside ) {
      v0.x += -( 1.4 + 0.5 * oppositeRatio ) * BoatDesign.OUTSIDE;

      v1.x += -0.9 * BoatDesign.OUTSIDE;
      v1.y += 0.9 * BoatDesign.OUTSIDE;

      v2.y += BoatDesign.OUTSIDE;

      v3.x += BoatDesign.OUTSIDE;
      v3.y += ( 0.9 - 0.1 * ratio ) * BoatDesign.OUTSIDE;
    }
  ...
  static getIntersectionVertices( blockHalfWidth = 0, liters = 1 ) {
    ...
    const outsideBottomY = -BoatDesign.BOAT_HEIGHT;
    const insideBottomY = -BoatDesign.BOAT_HEIGHT + BoatDesign.OUTSIDE;
    ...
    _.range( 0, frontSamples ).forEach( sample => {
      const y = outsideBottomY + BoatDesign.BOAT_HEIGHT * sample / ( frontSamples - 1 );
    ...
    _.range( 0, insideSamples ).forEach( sample => {
      const y = ( -BoatDesign.BOAT_HEIGHT + BoatDesign.OUTSIDE ) * sample / ( insideSamples - 1 );
  ...
  static createCrossSectionVertexArray() {
    return new Float32Array( BoatDesign.CROSS_SECTION_SAMPLES * 6 * 3 );
  }
  ...
  static fillCrossSectionVertexArray( y, liters, positionArray ) {
    ...
    for ( let i = 0; i < BoatDesign.CROSS_SECTION_SAMPLES; i++ ) {
      const t0 = i / BoatDesign.CROSS_SECTION_SAMPLES;
      const t1 = ( i + 1 ) / BoatDesign.CROSS_SECTION_SAMPLES;
  ...
  static getPrimaryGeometry( liters = 1 ) {
    ...
    const getRows = isInside => _.range( 0, heightSamples ).map( sample => {
      const designY = ( -BoatDesign.BOAT_HEIGHT + ( isInside ? BoatDesign.OUTSIDE : 0 ) ) * sample / ( heightSamples - 1 );
  ...
  static computeBoatData( samples = 1000 ) {
    ...
    const designInteriorBottom = -BoatDesign.BOAT_HEIGHT + BoatDesign.OUTSIDE;
    _.range( 0, samples ).forEach( i => {
      // unit area times the multiplier
      const y = ( i / ( samples - 1 ) - 1 ) * BoatDesign.BOAT_HEIGHT;
    ...
    const displacedVolume = externalAreaSum / samples * BoatDesign.BOAT_HEIGHT;
    const actualVolume = areaSum / samples * BoatDesign.BOAT_HEIGHT;
    ...
    const oneLiterHeight = BoatDesign.BOAT_HEIGHT * oneLiterMultiplier;
...
BoatDesign.DESIGN_ACTUAL_VOLUME = ${actualVolume};
...
BoatDesign.OUTSIDE = 3;
BoatDesign.BOAT_HEIGHT = 50;
BoatDesign.CROSS_SECTION_SAMPLES = 30;
...
BoatDesign.ONE_LITER_ACTUAL_VOLUME = BoatDesign.DESIGN_ACTUAL_VOLUME * BoatDesign.ONE_LITER_SCALE_MULTIPLIER * BoatDesign.ONE_LITER_SCALE_MULTIPLIER * BoatDesign.ONE_LITER_SCALE_MULTIPLIER;
