...
define( require => {
  ...
  class Boat extends Mass {
    ...
    static getAreaFromControlPoints( points ) {
      ...
      const positive = new Cubic( ...points );
      const line = new Line( points[ 3 ], flip( points[ 3 ] ) );
      const negative = new Cubic( ...points.slice().reverse().map( flip ) );

      return Math.abs(
        positive.getSignedAreaFragment() +
        line.getSignedAreaFragment() +
        negative.getSignedAreaFragment()
      );
    }
    ...
    static computeBoatData( samples = 1000 ) {
      ...
      _.range( 0, samples ).forEach( i => {
        ...
        const area = Boat.getAreaFromControlPoints( Boat.getControlPoints( Boat.getHeightRatioFromDesignY( y ), false ) );
        sliceAreas.push( area );
        sum += area;
      } );
      const originalVolume = sum / samples * BOAT_HEIGHT;
      const oneLiterMultiplier = Math.pow( originalVolume / desiredVolume, -1 / 3 );
...
`;
  // const ONE_LITER_INTERSECTION_CENTROID = new Vector2( ${centroid.x}, ${centroid.y} );
    }
