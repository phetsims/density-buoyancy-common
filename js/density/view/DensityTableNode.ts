    ...
    const gridBox = new GridBox( {
      children: [
        new Text( densityBuoyancyCommonStrings.material.name, {
          font: headerFont,
          maxWidth: 160,
          layoutOptions: optionize<ILayoutOptions, {}, ILayoutOptions>()( { column: 0, row: 0, xAlign: 'left' }, layoutOptions )
        } ),
        new Text( densityBuoyancyCommonStrings.densityKgL, {
          font: headerFont,
          maxWidth: 160,
          layoutOptions: optionize<ILayoutOptions, {}, ILayoutOptions>()( { column: 1, row: 0, xAlign: 'right' }, layoutOptions )
        } ),
        ...materials.map( ( material, index ) => new Text( material.name, {
          font: bodyFont,
          maxWidth: 200,
          layoutOptions: optionize<ILayoutOptions, {}, ILayoutOptions>()( { column: 0, row: index + 1, xAlign: 'left' }, layoutOptions )
        } ) ),
        ...materials.map( ( material, index ) => new Text( Utils.toFixed( material.density / 1000, 2 ), {
          font: bodyFont,
          maxWidth: 150,
          layoutOptions: optionize<ILayoutOptions, {}, ILayoutOptions>()( { column: 1, row: index + 1, xAlign: 'right' }, layoutOptions )
        } ) )
      ]
    } );
