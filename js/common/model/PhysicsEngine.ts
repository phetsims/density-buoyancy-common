...
abstract class PhysicsEngine {
   ...
   */
  abstract addBody( body: PhysicsEngineBody ): void;

  /**
   * Removes a body from the engine, so that it will not be tracked during the step anymore.
   */
  abstract removeBody( body: PhysicsEngineBody ): void;

  /**
   * Sets the mass of a body (and whether it can rotate, which for some engines needs to be set at the same time).
   */
  abstract bodySetMass( body: PhysicsEngineBody, mass: number, options?: { canRotate?: boolean } ): void;

  /**
   * Sets the provided matrix to the current transformation matrix of the body (to reduce allocations)
   */
  abstract bodyGetMatrixTransform( body: PhysicsEngineBody, matrix: Matrix3 ): Matrix3;

  /**
   * Sets the provided matrix to the current transformation matrix of the body (to reduce allocations)
   */
  abstract bodyGetStepMatrixTransform( body: PhysicsEngineBody, matrix: Matrix3 ): Matrix3;

  /**
   * Sets the position of a body.
   */
  abstract bodySetPosition( body: PhysicsEngineBody, position: Vector2 ): void;

  /**
   * Sets the rotation of a body.
   */
  abstract bodySetRotation( body: PhysicsEngineBody, rotation: number ): void;

  /**
   * Returns the velocity of a body.
   */
  abstract bodyGetVelocity( body: PhysicsEngineBody ): Vector2;

  /**
   * Sets the velocity of a body.
   */
  abstract bodySetVelocity( body: PhysicsEngineBody, velocity: Vector2 ): void;

  /**
   * Applies a given force to a body (should be in the post-step listener ideally)
   */
  abstract bodyApplyForce( body: PhysicsEngineBody, force: Vector2 ): void;

  /**
   * Returns the applied contact force computed in the last step.
   */
  abstract bodyGetContactForces( body: PhysicsEngineBody ): Vector2;

  /**
   * Returns the applied contact force computed in the last step (as a force on A from B).
   */
  abstract bodyGetContactForceBetween( bodyA: PhysicsEngineBody, bodyB: PhysicsEngineBody ): Vector2;

  /**
   * Resets the contact forces that have happened on a body to 0 after measurement.
   */
  abstract resetContactForces( body: PhysicsEngineBody ): void;

  /**
   * Returns a serialized form of a body
   */
  abstract bodyToStateObject( body: PhysicsEngineBody ): any;

  /**
   * Applies a given state object to a body.
   */
  abstract bodyApplyState( body: PhysicsEngineBody, obj: any ): void;

  /**
   * Returns a serialized form of a body
   */
  abstract bodyResetHidden( body: PhysicsEngineBody ): void;

  /**
   * Sets the previous position of a body to the current position
   */
  abstract bodySynchronizePrevious( body: PhysicsEngineBody ): void;

  /**
   * Creates a (static) ground body with the given vertices.
   */
  abstract createGround( vertices: Vector2[] ): PhysicsEngineBody;

  /**
   * Creates a (static) barrier body with the given vertices.
   */
  abstract createBarrier( vertices: Vector2[] ): PhysicsEngineBody;

  /**
   * Creates a (dynamic) box body, with the origin at the center of the box.
   */
  abstract createBox( width: number, height: number, isStatic?: boolean ): PhysicsEngineBody;

  /**
   * Updates the width/height of a box body.
   */
  abstract updateBox( body: PhysicsEngineBody, width: number, height: number ): void;

  /**
   * Creates a (dynamic) body, with the origin at the centroid.
   */
  abstract createFromVertices( vertices: Vector2[], workaround: boolean ): PhysicsEngineBody;

  /**
   * Updates the vertices of a dynamic vertex-based body.
   */
  abstract updateFromVertices( body: PhysicsEngineBody, vertices: Vector2[], workaround: boolean ): void;

  /**
   * Adds a listener to be called after each internal step.
   */
  abstract addPostStepListener( listener: ( dt: number ) => void ): void;

  /**
   * Removes a listener to be called after each internal step.
   */
  abstract removePostStepListener( listener: ( dt: number ) => void ): void;
   ...
   */
  abstract addPointerConstraint( body: PhysicsEngineBody, position: Vector2 ): void;

  /**
   * Updates a pointer constraint so that the body will essentially be dragged to the new position.
   */
  abstract updatePointerConstraint( body: PhysicsEngineBody, position: Vector2 ): void;

  /**
   * Removes a pointer constraint.
   */
  abstract removePointerConstraint( body: PhysicsEngineBody ): void;
}
