...
export default class BuoyancyBasicsCompareScreenView extends DensityBuoyancyScreenView<BuoyancyBasicsCompareModel> {
  ...
  public constructor( model: BuoyancyBasicsCompareModel, options: DensityBuoyancyScreenViewOptions ) {
    ...
    const customCompareScreenFormatting = [ model.primaryMass, model.secondaryMass ].map( mass => {
      return {
    ...
    model.secondaryMass.visibleProperty.link( visible => {
      ...
      densityBox.setReadoutItems( masses.map( ( mass, index ) => {
        return {
          ...
          readoutNameProperty: customCompareScreenFormatting[ index ].readoutNameProperty,
          readoutFormat: customCompareScreenFormatting[ index ].readoutFormat
        };
      } ) );
